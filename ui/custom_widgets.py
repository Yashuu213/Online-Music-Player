...
class SongCardWidget(QFrame):
    """Vertical card for the Home dashboard grid."""
    ...
    def init_ui(self):
        ...
        self.thumb_label.setStyleSheet("background: #111; border-radius: 12px;")
        self.thumb_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(self.thumb_label)
        
        self.title_label = QLabel(self.video_data.get('title', 'Unknown'))
        self.title_label.setStyleSheet("font-weight: 700; font-size: 13px; color: #FFF;")
        self.title_label.setWordWrap(True)
        self.title_label.setMaximumHeight(40)
        layout.addWidget(self.title_label)
        
        self.muted_label = QLabel("Click to Play")
        self.muted_label.setObjectName("MutedText")
...
class ArtistCardWidget(QFrame):
    """Circular profile widget for Popular Artists section."""
    clicked = pyqtSignal(str) # artist name
    ...
    def __init__(self, name, image_url=None):
        ...
        self.setStyleSheet("""
            ...
            #GlassPanel:hover { background: rgba(255, 255, 255, 0.05); border-radius: 15px; }
        """)
        self.setCursor(Qt.CursorShape.PointingHandCursor)
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # Circular Image with proper masking
        ...
        self.img_label.setText(self.name[0])
        self.img_label.setStyleSheet(self.img_label.styleSheet() + "font-size: 40px; font-weight: bold; color: #FFF;")
        
        layout.addWidget(self.img_label, alignment=Qt.AlignmentFlag.AlignCenter)
        
        self.name_label = QLabel(self.name)
        self.name_label.setObjectName("ArtistTitle")
        self.name_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(self.name_label)
        
        self.muted_label = QLabel("Artist")
        self.muted_label.setObjectName("MutedText")
        self.muted_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(self.muted_label)
    ...
    def set_image(self, pixmap):
        try:
            ...
            painter.setBrush(QBrush(pixmap.scaled(size, size, Qt.AspectRatioMode.KeepAspectRatioByExpanding, Qt.TransformationMode.SmoothTransformation)))
            painter.setPen(Qt.PenStyle.NoPen)
            painter.drawEllipse(0, 0, size, size)
            painter.end()
            
            self.img_label.setPixmap(rounded)
            self.img_label.setText("") # Clear fallback text
        except:
             pass
